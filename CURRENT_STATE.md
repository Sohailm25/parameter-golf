...
**Status:** in_progress
**Current phase:** Phase 1 - the standalone research scaffold is established, the official challenge code is already the workspace root, and the repo now has `bd` state, hooks, tests, PR-intelligence workflow, public-signal hooks, a dynamic-eval review artifact, append-only telemetry, local paper caching, a `main` source-of-truth branch, a canonical `START_HERE.md`, a launch/promote runner, minimal validation helpers, and the first real local baseline smoke/workflow artifact. No golden-set candidate exists yet.
...
- `observed`: newer PRs through `#96` keep reinforcing the same live frontier: sliding-window evaluation, longer context, selective precision, and better local sweep tooling still matter enough that the PR-intelligence loop is already paying for itself.
